...
#tikz('state_tree')
cairo_pdf(paste('graphics/',feat.old,'.pdf',sep=''),height=15,width=10)
...
nodelabels(pie=states.agg[non.advent,],node=non.advent,cex=.4,col=NULL,piecol=node.colors)

feat.title <- paste(unlist(strsplit(unlist(strsplit(feat,';'))[1],"|",fixed=T))[1],unlist(strsplit(unlist(strsplit(feat,';'))[1],"|",fixed=T))[2],sep=' — ')
print(feat.title)
#CAREFUL!!
legend('bottomleft',legend=levels(as.factor(diacl.data[,feat.ind])),title=feat.title,fill=node.colors,bty='o',cex=.75)
#title(gsub(feat,'|','; '))
#title(feat,cex=.25)
