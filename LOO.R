...
t = as.integer(commandArgs(trailingOnly=TRUE)[2])
print(feat.ind)
...
model.code <- "data {
  ...
  int<lower=1> L;                       //tip to leave out
  //int<lower=1> C;                       //branch of tip
  }
transformed data {
  int<lower=0,upper=1> tiplik_heldout[T,F];
  tiplik_heldout = tiplik;
  for (f in 1:F) {
    tiplik_heldout[L,f] = 1;            //set likelihoods to 1 for held out tip
  }
}
parameters {
...
model {
  ...
  for (i in 1:F*(F-1)) {
    R[i] ~ uniform(0,5);                //shape/rate parameterization
  }
  for (t in 1:T) {
    for (f in 1:F) {
...
generated quantities {
  ...
  real Z;
  matrix[F,F] P;
  vector[F] heldout_state;
  int b_;
  for (t in 1:T) {
    for (f in 1:F) {
      lambda[t,f] = log(tiplik_heldout[t,f]);
    }
  }
  for (n in T+1:N) {
    for (f in 1:F) {
  ...
  pi_matrix = matrix_exp(1000*Q);
  for (d in 1:F) {                      //dirty way of approximating
    pi[d] = pi_matrix[1,d];             //stationary probabilities of
  }                                     //each character state;
  for (f in 1:F) {
    phi[f] = log(pi[f]) + lambda[parent[B],f];
  }
  Z = log_sum_exp(phi);
  for (f in 1:F) {
    phi[f] = exp(phi[f] - Z);
  }
  z[parent[B]] = multinomial_rng(to_vector(phi),1);
  b_ = B;
  while (b_ > 0) {
	P = matrix_exp(brlen[b_]*Q);
	for (f in 1:F) {
	  phi[f] = log(dot_product(to_vector(z[parent[b_]]),P[,f])) + lambda[child[b_],f];
	}
	Z = log_sum_exp(phi);
	for (f in 1:F) {
	  phi[f] = exp(phi[f] - Z);
	}
	z[child[b_]] = multinomial_rng(to_vector(phi),1);
	b_ -= 1;
  }
  heldout_state = to_vector(z[L]);
}"

#accuracies <- NULL

lang <- IE.trees[[1]]$tip.label[t]
print(lang)
...
#for (t in 1:T) {
	...
	data.list <- list(N=N,
	      ...
	      F=ncol(curr.states),
		  L=t)
	fit <- stan(model_code=model.code,data=data.list,chains=1)
	print(sum(colMeans(extract(fit)$heldout_state)*curr.states[t,]))
#	accuracies <- rbind(accuracies,c(colnames(diacl.data)[feat.ind],rownames(curr.states)[t],sum(colMeans(extract(fit)$heldout_state)*curr.states[t,])))
	#}

if (!('accuracies' %in% dir('.'))) {
  dir.create('accuracies')
}

print(rownames(curr.states)[t])
    
write.table(t(c(colnames(diacl.data)[feat.ind],
              rownames(curr.states)[t],
			  sum(colMeans(extract(fit)$heldout_state)*curr.states[t,])))
	,file=paste('accuracies/heldout_acc_',colnames(diacl.data)[feat.ind],'.tsv',sep=''),row.names=F,col.names=F,quote=F,sep='\t',append=T)
#write.table(accuracies,file=paste('accuracies/heldout_acc_',colnames(diacl.data)[feat.ind],'.tsv',sep=''),row.names=F,col.names=F,quote=F,sep='\t')
