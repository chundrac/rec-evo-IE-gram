...
if (!dir.exists('stanfits_dirprior/')) {
	dir.create('stanfits_dirprior/')
}
...
transformed data {
  vector[F] pi_prior;
  pi_prior = rep_vector(1,F);
}
parameters {
  real<lower=0> R[F*(F-1)];             //rates to be put into matrix
  simplex[F] pi;                        //prior at root
  }
...
for (i in 1:length(IE.trees)) {
	...
	save.image(file=paste('stanfits_dirprior/',colnames(diacl.data)[feat.ind],'.Rdata',sep=''))
   }
